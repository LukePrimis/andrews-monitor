...
func (a *AndrewsTask) getPushResponse() (string, error) {
	var data = strings.NewReader(`sid=` + url.PathEscape(a.Sid))
	req, err := http.NewRequest("POST", "https://api-bf620061.duosecurity.com/frame/status/"+a.Txid, data)
	if err != nil {
		return "", nil
	}
	...
	a.Auth = txmap["cookie"].(string)

	fmt.Println("got auth token", a.Auth)
	return "got resp", nil
}
